...
#     2023-Sep-11: renmae function to conductance_pressure to be general
# Bug fixes
...
"""
    ...
    conductance_pressure(vc::ComplexVC{FT}, kr::FT) where {FT<:AbstractFloat}
    conductance_pressure(vc::LogisticVC{FT}, kr::FT) where {FT<:AbstractFloat}
    conductance_pressure(vc::PowerVC{FT}, kr::FT) where {FT<:AbstractFloat}
    conductance_pressure(vc::WeibullVC{FT}, kr::FT) where {FT<:AbstractFloat}
...
"""
function conductance_pressure end
...
conductance_pressure(vc::ComplexVC{FT}, kr::FT) where {FT<:AbstractFloat} = (
    ...
    _p_crit::FT = 0;
    for _vc in VCS
        _p_crit = min(_p_crit, conductance_pressure(_vc, kr));
    end;
