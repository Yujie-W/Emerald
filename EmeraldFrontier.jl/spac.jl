...
function spac(gm_dict::Dict, config::SPACConfiguration{FT}) where {FT<:AbstractFloat}
    ...
    _spac = MultiLayerSPAC{FT}(air_bounds = _air_bounds, latitude = gm_dict["LATITUDE"], longitude = gm_dict["LONGITUDE"], soil_bounds = [0, -0.1, -0.35, -1, -3], zs = [-2, _z_canopy/2, _z_canopy]);
    _spac.SOIL.COLOR = gm_dict["SOIL_COLOR"];
