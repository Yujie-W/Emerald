...
"""
Base.@kwdef mutable struct HyperspectralMLCanopyOpticalProperty{FT,DIMS}
    ...
    fo::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "Conversion factor fs for angles from solar at different inclination and azimuth angles"
    fs::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    ...
    ϵ::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Effective reflectance for diffuse->diffuse"
    ρ_dd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY+1)
    "Effective reflectance for longwave radiation"
    ρ_lw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY+1)
    "Effective reflectance for directional->diffuse"
    ρ_sd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY+1)
    "Backward scattering coefficient for diffuse->diffuse at different layers and wavelength bins"
    σ_ddb::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Forward scattering coefficient for diffuse->diffuse at different layers and wavelength bins"
    σ_ddf::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Backward scattering coefficient for diffuse->observer at different layers and wavelength bins"
    σ_dob::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Forward scattering coefficient for diffuse->observer at different layers and wavelength bins"
    σ_dof::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Backward scattering coefficient for solar directional->diffuse at different layers and wavelength bins"
    σ_sdb::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Forward scattering coefficient for solar directional->diffuse at different layers and wavelength bins"
    σ_sdf::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Bidirectional from solar to observer scattering coefficient at different layers and wavelength bins"
    σ_so::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Effective tranmittance for diffuse->diffuse"
    τ_dd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Effective tranmittance for longwave radiation"
    τ_lw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Effective tranmittance for solar directional->diffuse"
    τ_sd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)

    # Cache variables
    "cos(inclination) * cos(vza) at different inclination angles"
    _Co::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "cos(inclination) * cos(sza) at different inclination angles"
    _Cs::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "sin(inclination) * sin(vza) at different inclination angles"
    _So::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "sin(inclination) * sin(sza) at different inclination angles"
    _Ss::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "abs of fo"
    _abs_fo::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "abs of fs"
    _abs_fs::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "abs of fs * fo"
    _abs_fs_fo::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "Weighted sum of cos²(inclination)"
    _bf::FT = 0
    "Cosine of Θ_AZI - raa"
    _cos_θ_azi_raa::Vector{FT} = zeros(FT, DIMS.DIM_AZI)
    "fo * cos Θ_INCL"
    _fo_cos_θ_incl::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "fs * cos Θ_INCL"
    _fs_cos_θ_incl::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "fs * fo"
    _fs_fo::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "Outgoing beam extinction coefficient weights at different inclination angles"
    _ko::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "Solar beam extinction coefficient weights at different inclination angles"
    _ks::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "Upwelling matrix for SIF excitation"
    _mat⁺::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_SIFE)
    "Downwelling matrix for SIF excitation"
    _mat⁻::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_SIFE)
    "Backward scattering coefficients at different inclination angles"
    _sb::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "Forward scattering coefficients at different inclination angles"
    _sf::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "Temporary cache used for matrix adding up purpose (DIM_INCL * DIMS.DIM_AZI)"
    _tmp_mat_incl_azi_1::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "Temporary cache used for matrix adding up purpose (DIM_INCL * DIMS.DIM_AZI)"
    _tmp_mat_incl_azi_2::Matrix{FT} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI)
    "Temporary cache used for vector operations (DIM_AZI)"
    _tmp_vec_azi::Vector{FT} = zeros(FT, DIMS.DIM_AZI)
    "Temporary cache used for vector operations (DIM_CANOPY)"
    _tmp_vec_layer::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Cache variable to store the SIF information"
    _tmp_vec_sif_1::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache variable to store the SIF information"
    _tmp_vec_sif_2::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache variable to store the SIF information"
    _tmp_vec_sif_3::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache variable to store the SIF information"
    _tmp_vec_sif_4::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache variable to store the SIF information"
    _tmp_vec_sif_5::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache variable to store the SIF information"
    _tmp_vec_sif_6::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache variable to store the SIF excitation information"
    _tmp_vec_sife_1::Vector{FT} = zeros(FT, DIMS.DIM_SIFE)
    "Cache variable to store the SIF excitation information"
    _tmp_vec_sife_2::Vector{FT} = zeros(FT, DIMS.DIM_SIFE)
    "Cache variable to store the SIF excitation information"
    _tmp_vec_sife_3::Vector{FT} = zeros(FT, DIMS.DIM_SIFE)
    "Temporary cache used for vector operations (DIM_WL)"
    _tmp_vec_λ::Vector{FT} = zeros(FT, DIMS.DIM_WL)
    "Reflectance for diffuse->diffuse at each canopy layer"
    _ρ_dd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Reflectance for longwave radiation at each canopy layer"
    _ρ_lw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Reflectance for solar directional->diffuse at each canopy layer"
    _ρ_sd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Tranmittance for diffuse->diffuse at each canopy layer"
    _τ_dd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Tranmittance for longwave radiation at each canopy layer"
    _τ_lw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Tranmittance for solar directional->diffuse at each canopy layer"
    _τ_sd::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
...
"""
Base.@kwdef mutable struct BroadbandSLCanopyRadiationProfile{FT,DIMS} <: AbstractCanopyRadiationProfile{FT}
    ...
    _k_diffuse::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
    "Extinction coefficient for direct radiation at different leaf inclination angles"
    _k_direct::Vector{FT} = zeros(FT, DIMS.DIM_INCL)
end
...
"""
Base.@kwdef mutable struct HyperspectralMLCanopyRadiationProfile{FT,DIMS} <: AbstractCanopyRadiationProfile{FT}
    # Diagnostic variables
    "Albedo towards the viewing direction"
    albedo::Vector{FT} = zeros(FT, DIMS.DIM_WL)
    "Mean APAR for shaded leaves `[μmol m⁻² s⁻¹]`"
    apar_shaded::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "APAR for sunlit leaves `[μmol m⁻² s⁻¹]`"
    apar_sunlit::Array{FT,3} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI, DIMS.DIM_CANOPY)
    "Downwelling diffuse short-wave radiation at each canopy layer boundary `[mW m⁻² nm⁻¹]`"
    e_diffuse_down::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY+1)
    "Upwelling diffuse short-wave radiation at each canopy layer boundary `[mW m⁻² nm⁻¹]`"
    e_diffuse_up::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY+1)
    "Solar directly radiation at each canopy layer boundary `[mW m⁻² nm⁻¹]`"
    e_direct::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY+1)
    "Net diffuse radiation at each canopy layer for APAR `[mW m⁻² nm⁻¹]`"
    e_net_diffuse::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Net direct radiation at each canopy layer for APAR `[mW m⁻² nm⁻¹]`"
    e_net_direct::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Total radiation towards the viewing direction `[mW m⁻² nm⁻¹]`"
    e_o::Vector{FT} = zeros(FT, DIMS.DIM_WL)
    "Sum diffuse radiation at each canopy layer for PAR `[mW m⁻² nm⁻¹]`"
    e_sum_diffuse::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Sum direct radiation at each canopy layer for PAR `[mW m⁻² nm⁻¹]`"
    e_sum_direct::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY)
    "Radiation towards the viewing direction per layer (including soil) `[mW m⁻² nm⁻¹]`"
    e_v::Matrix{FT} = zeros(FT, DIMS.DIM_WL, DIMS.DIM_CANOPY+1)
    ...
    par_shaded::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "PAR for sunlit leaves (before absorption) `[μmol m⁻² s⁻¹]`"
    par_sunlit::Array{FT,3} = zeros(FT, DIMS.DIM_INCL, DIMS.DIM_AZI, DIMS.DIM_CANOPY)
    "Longwave energy flux from leaves per leaf area (one side) `[W m⁻²]`"
    r_lw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Downwelling longwave energy flux `[W m⁻²]`"
    r_lw_down::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY+1)
    "Upwelling longwave energy flux `[W m⁻²]`"
    r_lw_up::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY+1)
    "Net longwave energy absorption for all leaves `[W m⁻²]`"
    r_net_lw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Net shortwave energy absorption for all leaves `[W m⁻²]`"
    r_net_sw::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Net shortwave energy absorption for shaded leaves `[W m⁻²]`"
    r_net_sw_shaded::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Net shortwave energy absorption for sunlit leaves `[W m⁻²]`"
    r_net_sw_sunlit::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Downwelling SIF for sunlit leaves at each wavelength for a layer"
    s_layer_down::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY)
    "Upwelling SIF for sunlit leaves at each wavelength for a layer"
    s_layer_up::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY)
    "Downwelling SIF"
    sif_down::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY+1)
    "SIF at observer direction"
    sif_obs::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "SIF at observer direction from shaded APAR"
    sif_obs_shaded::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "SIF at observer direction from scattering"
    sif_obs_scatter::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "SIF at observer direction from soil reflection"
    sif_obs_soil::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "SIF at observer direction from sunlit APAR"
    sif_obs_sunlit::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Upwelling SIF"
    sif_up::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY+1)

    # Cache variables
    "Mean APAR for shaded leaves per wavelength `[μmol m⁻² s⁻¹ nm⁻¹]`"
    _apar_shaded::Vector{FT} = zeros(FT, DIMS.DIM_PAR)
    "APAR for sunlit leaves per wavelength `[μmol m⁻² s⁻¹ nm⁻¹]`"
    _apar_sunlit::Vector{FT} = zeros(FT, DIMS.DIM_PAR)
    "Mean PAR for shaded leaves per wavelength (before absorption) `[μmol m⁻² s⁻¹ nm⁻¹]`"
    _par_shaded::Vector{FT} = zeros(FT, DIMS.DIM_PAR)
    "PAR for sunlit leaves per wavelength (before absorption) `[μmol m⁻² s⁻¹ nm⁻¹]`"
    _par_sunlit::Vector{FT} = zeros(FT, DIMS.DIM_PAR)
    "Mean APAR for shaded leaves for photosynthesis per wavelength `[μmol m⁻² s⁻¹ nm⁻¹]`"
    _ppar_shaded::Vector{FT} = zeros(FT, DIMS.DIM_PAR)
    "APAR for sunlit leaves for photosynthesis per wavelength `[μmol m⁻² s⁻¹ nm⁻¹]`"
    _ppar_sunlit::Vector{FT} = zeros(FT, DIMS.DIM_PAR)
    "Downwelling longwave energy flux cache `[W m⁻²]`"
    _r_emit_down::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY)
    "Upwelling longwave energy flux cache `[W m⁻²]`"
    _r_emit_up::Vector{FT} = zeros(FT, DIMS.DIM_CANOPY+1)
    "Downwelling SIF for sunlit leaves at each wavelength"
    _s_emit_down::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY)
    "Upwelling SIF for sunlit leaves at each wavelength"
    _s_emit_up::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY+1)
    "Downwelling SIF for shaded leaves at each wavelength"
    _s_shaded_down::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Upwelling SIF for shaded leaves at each wavelength"
    _s_shaded_up::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Downwelling SIF for sunlit leaves at each wavelength"
    _s_sunlit_down::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Upwelling SIF for sunlit leaves at each wavelength"
    _s_sunlit_up::Vector{FT} = zeros(FT, DIMS.DIM_SIF)
    "Cache to compute SIF at observer direction from shaded APAR"
    _sif_obs_shaded::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY)
    "Cache to compute SIF at observer direction from scattering"
    _sif_obs_scatter::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY)
    "Cache to compute SIF at observer direction from sunlit APAR"
    _sif_obs_sunlit::Matrix{FT} = zeros(FT, DIMS.DIM_SIF, DIMS.DIM_CANOPY)
end
...
"""
Base.@kwdef mutable struct BroadbandSLCanopy{FT,DIMS} <: AbstractCanopy{FT}
    # Embedded structures
    "Leaf inclination angle distribution function algorithm"
    LIDF::Union{VerhoefLIDF{FT}} = VerhoefLIDF{FT}()
    "Canopy radiation profiles"
    RADIATION::BroadbandSLCanopyRadiationProfile{FT} = BroadbandSLCanopyRadiationProfile{FT}(DIM_INCL = DIMS.DIM_INCL)
...
Base.@kwdef mutable struct HyperspectralMLCanopy{FT,DIMS} <: AbstractCanopy{FT}
    ...
    OPTICS::HyperspectralMLCanopyOpticalProperty{FT,DIMS} = HyperspectralMLCanopyOpticalProperty{FT,DIMS}()
    "Canopy radiation profiles"
    RADIATION::HyperspectralMLCanopyRadiationProfile{FT,DIMS} = HyperspectralMLCanopyRadiationProfile{FT,DIMS}()
