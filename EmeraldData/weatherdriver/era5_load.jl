...
era5_weather_drivers(wd::ERA5SingleLevelsDriver, year::Int, nx::Int, ind::Int) = (
    ...
    wd_p_atm = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.P_ATM[2])_SL_$(year)_$(nx)X.nc", wd.P_ATM[1], ind);
    wd_t_air = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.T_AIR[2])_SL_$(year)_$(nx)X.nc", wd.T_AIR[1], ind);
    wd_t_dew = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.T_DEW[2])_SL_$(year)_$(nx)X.nc", wd.T_DEW[1], ind);
    wd_windu = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.WINDU[2])_SL_$(year)_$(nx)X.nc", wd.WINDU[1], ind);
    wd_windv = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.WINDV[2])_SL_$(year)_$(nx)X.nc", wd.WINDV[1], ind);
    wd_l_all = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.L_RAD[2])_SL_$(year)_$(nx)X.nc", wd.L_RAD[1], ind);
    wd_s_all = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.S_ALL[2])_SL_$(year)_$(nx)X.nc", wd.S_ALL[1], ind);
    wd_s_dir = read_nc("$(ERA5_FOLDER)/reprocessed/$(wd.S_DIR[2])_SL_$(year)_$(nx)X.nc", wd.S_DIR[1], ind);
    wd_s_dif = wd_s_all .- wd_s_dir;
    wd_vpd   = saturation_vapor_pressure.(wd_t_air) .- saturation_vapor_pressure.(wd_t_dew);
    wd_wind  = sqrt.(wd_windu .^ 2 .+ wd_windv .^ 2);
