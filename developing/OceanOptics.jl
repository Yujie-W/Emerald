...
using ..DataCenter.Jld2: read_jld2
...
# read the radiation
df = Emerald.DataCenter.Jld2.read_jld2(Emerald.EmeraldLand.Namespace.LAND_ARTIFACT, Emerald.EmeraldLand.Namespace.OLD_PHI_2021_1NM);
rad = df.E_DIR .+ df.E_DIFF;
rad_photon = PkgUtility.UniversalConstants.energy_to_photon.(owl.Λ, rad);
sif_photon = owl.sif_mat * rad_photon;
sif = PkgUtility.UniversalConstants.photon_to_energy.(owl.Λ, sif_photon);

df_owl = DataFrames.DataFrame(WL = owl.Λ, NR = owl.NR, REFL = owl.ρ_surface, SIF = sif, PHI = owl.Φ_PS);
Emerald.DataCenter.Text.save_csv!(df_owl, "sif.csv");
