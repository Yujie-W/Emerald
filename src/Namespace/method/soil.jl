...
BrooksCorey{FT}(catg::Int) where {FT} = (
    ...
    return BrooksCorey{FT}(
                B     = SOIL_TEXTURE.BB[catg],
                K_MAX = SOIL_TEXTURE.SATDK[catg] / GRAVITY(FT) * 1e6 / M_H₂O(FT),
                TYPE  = SOIL_TEXTURE.NAME[catg],
                Ψ_SAT = SOIL_TEXTURE.SATPSI[catg] * ρ_H₂O(FT) * GRAVITY(FT) * 1e-6,
                Θ_SAT = SOIL_TEXTURE.MAXSMC[catg],
                Θ_RES = SOIL_TEXTURE.REFSMC[catg]
    )
);
