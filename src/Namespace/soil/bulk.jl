...
#     2024-Feb-27: move albedo method to SPACConfig
#
#######################################################################################################################################################################################################
"""
...
SoilBulkAuxil(config::SPACConfig{FT}, n_soil::Int) where {FT} = SoilBulkAuxil{FT}(
            e_net_dif = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ)),
            e_net_dir = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ)),
            ρ_sw      = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ)),
...
SoilBulk(config::SPACConfig{FT}, n_soil::Int) where {FT} = SoilBulk{FT}(auxil = SoilBulkAuxil(config, n_soil));
