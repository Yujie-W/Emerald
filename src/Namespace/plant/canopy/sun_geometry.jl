...
Base.@kwdef mutable struct SunGeometryAuxil{FT}
    ...
    "Reflectance for solar directional->diffuse at each canopy layer before reabsorption"
    ρ_sd_layer_0::Matrix{FT}
    "Tranmittance for solar directional->diffuse at each canopy layer before reabsorption"
    τ_sd_layer_0::Matrix{FT}
    "Reflectance for solar directional->diffuse at each canopy layer after reabsorption"
    ρ_sd_layer_1::Matrix{FT}
    "Tranmittance for solar directional->diffuse at each canopy layer after reabsorption"
    τ_sd_layer_1::Matrix{FT}
    "Reflectance for solar directional->diffuse at each canopy layer after reabsorption and denominator correction"
    ρ_sd_layer::Matrix{FT}
    "Tranmittance for solar directional->diffuse at each canopy layer after reabsorption and denominator correction"
...
SunGeometryAuxil(config::SPACConfig{FT}, n_layer::Int) where {FT} = (
    ...
    return SunGeometryAuxil{FT}(
                ...
                ρ_sd_layer_0     = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                τ_sd_layer_0     = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                ρ_sd_layer_1     = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                τ_sd_layer_1     = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                ...
                ρ_sd             = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer + 1),
                τ_sd             = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                albedo           = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ)),
                e_difꜜ           = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer + 1),
                e_difꜛ           = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer + 1),
                e_difꜛ_layer     = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer + 1),
                e_dirꜜ           = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer + 1),
                e_net_dif        = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                e_net_dir        = zeros(FT, length(config.CONSTANTS.SPECTRA.Λ), n_layer),
                r_net_sw_leaf    = zeros(FT, n_layer),
                r_net_sw_stem    = zeros(FT, n_layer),
                e_sif_chl        = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF), n_layer),
                e_sifꜜ_layer     = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF), n_layer),
                e_sifꜛ_layer     = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF), n_layer),
                e_sifꜛ_layer_sum = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF), n_layer),
                e_sifꜜ_emit      = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF), n_layer),
                ...
                ppar_sunlit      = zeros(FT, config.DIMENSIONS.DIM_INCL, config.DIMENSIONS.DIM_AZI, n_layer),
                ...
                ppar_index       = zeros(Int, config.DIMENSIONS.DIM_INCL, config.DIMENSIONS.DIM_AZI, n_layer),
                _e_dif_sife      = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIFE)),
                _e_dir_sife      = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIFE)),
                _e_dif_sif       = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF)),
                _e_dir_sif       = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF)),
                _e_dif_shaded    = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF)),
                _e_dif_sunlit    = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF)),
                _e_dir_sunlit    = zeros(FT, length(config.CONSTANTS.SPECTRA.IΛ_SIF)),
