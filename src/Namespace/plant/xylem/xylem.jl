...
XylemHydraulicsState(config::SPACConfig{FT}) where {FT} = XylemHydraulicsState{FT}(
            p_history = zeros(FT, config.DIMENSIONS.DIM_XYLEM),
            v_storage = ones(FT, config.DIMENSIONS.DIM_XYLEM) ./ config.DIMENSIONS.DIM_XYLEM .* (0.1 * ρ_H₂O() / M_H₂O()),
);
...
XylemHydraulicsAuxilNSS(config::SPACConfig{FT}) where {FT} = XylemHydraulicsAuxilNSS{FT}(
            flow        = zeros(FT, config.DIMENSIONS.DIM_XYLEM + 1),
            flow_buffer = zeros(FT, config.DIMENSIONS.DIM_XYLEM),
            p_storage   = zeros(FT, config.DIMENSIONS.DIM_XYLEM),
            pressure    = zeros(FT, config.DIMENSIONS.DIM_XYLEM + 1)
);
...
XylemHydraulicsAuxilSS(config::SPACConfig{FT}) where {FT} = XylemHydraulicsAuxilSS{FT}(pressure  = zeros(FT, config.DIMENSIONS.DIM_XYLEM + 1));
...
XylemHydraulics(config::SPACConfig{FT}) where {FT} = XylemHydraulics{FT}(
            ...
            auxil = XylemHydraulicsAuxilSS(config)
);
