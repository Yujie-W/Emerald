...
MODIS_BAND_REFL(config::SPACConfig{FT}, spac::BulkSPAC{FT}, i::Int; steps::Int = 4, weighted::Bool = false) where {FT} =
    MODIS_BAND_REFL(spac.canopy, config.CONSTANTS.SPECTRA, spac.meteo.rad_sw, i; steps = steps, weighted = weighted);
...
MODIS_RED(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT} = MODIS_BAND_REFL(config, spac, 1; steps = 6, weighted = weighted);
MODIS_NIR(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT} = MODIS_BAND_REFL(config, spac, 2; steps = 6, weighted = weighted);
MODIS_BLUE(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT} = MODIS_BAND_REFL(config, spac, 3; steps = 4, weighted = weighted);
...
"""
function MODIS_EVI(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
...
"""
    ...
    MODIS_EVI2(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
...
"""
function MODIS_EVI2(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
    red = MODIS_RED(config, spac; weighted = weighted);
    nir = MODIS_NIR(config, spac; weighted = weighted);
...
"""
    ...
    MODIS_LSWI(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
...
"""
function MODIS_LSWI(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
    nir  = MODIS_NIR(config, spac; weighted = weighted);
...
"""
    ...
    MODIS_NDVI(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
...
"""
function MODIS_NDVI(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
    red = MODIS_RED(config, spac; weighted = weighted);
    nir = MODIS_NIR(config, spac; weighted = weighted);
...
"""
    ...
    MODIS_NIRv(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
...
"""
function MODIS_NIRv(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
    red = MODIS_RED(config, spac; weighted = weighted);
    nir = MODIS_NIR(config, spac; weighted = weighted);
...
"""
    ...
    MODIS_NIRvR(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
...
"""
function MODIS_NIRvR(config::SPACConfig{FT}, spac::BulkSPAC{FT}; weighted::Bool = false) where {FT}
    nir_rad = read_spectrum(config.CONSTANTS.SPECTRA.Λ, spac.canopy.sensor_geometry.auxil.e_sensor, FT(MODIS_BAND_2[1]), FT(MODIS_BAND_2[2]); steps=6);
