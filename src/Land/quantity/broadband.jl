...
"""
    ...
    LEAF_RT_BB(config::SPACConfig{FT}, lbio::LeafBio{FT}) where {FT}

Return leaf PAR and NIR reflectance and transmittance from leaf level spectra, given
- `config`: a `SPACConfig` object containing the reference radiation spectra
...
"""
    ...
    ALBEDO_BB(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
