...
"""
    ...
    plant_growth!(config::SPACConfig{FT}, spac::BulkSPAC{FT})

Allocate carbon to different plant organs, given
- `spac` `BulkSPAC` structure

"""
function plant_growth!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    if !config.FEATURES.ALLOW_XYLEM_GROWTH
        return nothing
    end;
