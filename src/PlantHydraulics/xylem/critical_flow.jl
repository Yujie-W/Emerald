...
"""
    ...
    critical_flow(config::SPACConfig{FT}, xylem::XylemHydraulics{FT}, cache::SPACCache{FT}, t::FT, ini::FT = FT(0.5)) where {FT}
...
"""
function critical_flow(config::SPACConfig{FT}, xylem::XylemHydraulics{FT}, cache::SPACCache{FT}, t::FT, ini::FT = FT(0.5)) where {FT}
    if xylem.state.asap <= 0
        return FT(0)
    end;

    # run the pressure profile calculation only if xylem area > 0
    (; KR_THRESHOLD) = config.FEATURES;
