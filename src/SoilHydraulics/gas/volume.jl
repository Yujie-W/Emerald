"""
    ...
    volume_balance!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
...
"""
function volume_balance!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    (; PRESCRIBE_AIR) = config.FEATURES;
    air = spac.airs[1];
    soils = spac.soils;
    top_soil = soils[1];
    ...
    for i in length(soils)-1:-1:1
        ...
        nmax_j = (air.state.p_air - saturation_vapor_pressure(soil_j.auxil.t, soil_j.auxil.ψ * 1000000)) * soil_j.auxil.δz *
                 max(0, soil_j.trait.vc.Θ_SAT - soil_j.state.θ - soil_j.state.θ_ice) / (GAS_R(FT) * soil_j.auxil.t);
        ...
        if (nmax_j > ndry_j) && (ndry_i > 0)
            ...
            soil_j.state.Σe += n_mass * CP_D_MOL(FT) * (soil_i.auxil.t - T₀(FT)) / soil_j.auxil.δz;
            ...
            soil_i.state.Σe -= n_mass * CP_D_MOL(FT) * (soil_i.auxil.t - T₀(FT)) / soil_i.auxil.δz;
        end;
        ...
        if (nmax_j > ndry_j) && (soil_i.state.θ >= soil_i.trait.vc.Θ_SAT)
            θ_fc_up = soil_θ(soil_i.trait.vc, -1 * ρg_MPa(FT) * soil_i.auxil.δz / relative_surface_tension(soil_i.auxil.t));
            v_mass = min((nmax_j - ndry_j) * GAS_R(FT) * soil_j.auxil.t / air.state.p_air, (soil_i.trait.vc.Θ_SAT - θ_fc_up) * soil_i.auxil.δz);
            soil_j.state.θ += v_mass / soil_j.auxil.δz;
            soil_i.state.θ -= v_mass / soil_i.auxil.δz;
            soil_j.state.Σe += v_mass * ρ_H₂O(FT) * CP_L(FT) * (soil_i.auxil.t - T₀(FT)) / soil_j.auxil.δz;
            soil_i.state.Σe -= v_mass * ρ_H₂O(FT) * CP_L(FT) * (soil_i.auxil.t - T₀(FT)) / soil_i.auxil.δz;
        end;
        ...
        if (nmax_j < ndry_j) && (ndry_j > 0)
            ...
            soil_i.state.Σe += n_mass * CP_D_MOL(FT) * (soil_j.auxil.t - T₀(FT)) / soil_i.auxil.δz;
            ...
            soil_j.state.Σe -= n_mass * CP_D_MOL(FT) * (soil_j.auxil.t - T₀(FT)) / soil_j.auxil.δz;
        end;
    end;
    ...
    s_max = (air.state.p_air - saturation_vapor_pressure(top_soil.auxil.t, top_soil.auxil.ψ * 1000000)) * top_soil.auxil.δz *
            max(0, top_soil.trait.vc.Θ_SAT - top_soil.state.θ - top_soil.state.θ_ice) / (GAS_R(FT) * top_soil.auxil.t);
    ...
    if s_max > s_dry
        ...
        top_soil.state.Σe += n_mass * CP_D_MOL(FT) * (air.auxil.t - T₀(FT)) / top_soil.auxil.δz;
        if !PRESCRIBE_AIR
    ...
    elseif s_max < s_dry
        ...
        top_soil.state.Σe -= n_mass * CP_D_MOL(FT) * (top_soil.auxil.t - T₀(FT)) / top_soil.auxil.δz;
        for ins in 1:5
            top_soil.state.ns[ins] = max( top_soil.state.ns[ins], 0 );
        end;
        if !PRESCRIBE_AIR
