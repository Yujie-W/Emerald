...
"""
    ...
    trace_gas_diffusion!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
...
"""
function trace_gas_diffusion!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    (; TRACE_AIR, TRACE_CH₄, TRACE_CO₂, TRACE_H₂O, TRACE_N₂, TRACE_O₂) = config.CONSTANTS;
    ...
    v_gas = max(0, soils[1].trait.vc.Θ_SAT - soils[1].state.θ - soils[1].state.θ_ice) * soils[1].auxil.δz;
    if v_gas > 0
    ...
    for i in 1:n_soil-1
        # for gas 1,2,4,5
        v_gas_i = max(0, soils[i  ].trait.vc.Θ_SAT - soils[i  ].state.θ - soils[i  ].state.θ_ice) * soils[i  ].auxil.δz;
        v_gas_j = max(0, soils[i+1].trait.vc.Θ_SAT - soils[i+1].state.θ - soils[i+1].state.θ_ice) * soils[i+1].auxil.δz;
