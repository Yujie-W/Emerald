...
function soil_water_infiltration!(spac::BulkSPAC{FT}) where {FT}
    ...
    for i in 1:N
        ...
        if soils[i].auxil.ψ < -50 && soils[i+1].auxil.ψ < -50
            sbulk.auxil.δψ[i] = 0;
            sbulk.auxil.q[i] = 0;
        end;
        ...
        if (soils[i].state.θ >= soils[i].trait.vc.Θ_SAT) && (soils[i+1].state.θ > soils[i+1].trait.vc.Θ_SAT)
            sbulk.auxil.q[i] = -1 * (soils[i+1].state.θ - soils[i+1].trait.vc.Θ_SAT) * soils[i+1].auxil.δz * ρ_H₂O(FT) / M_H₂O(FT);
        end;
    end;
    ...
    for i in 1:N
        soils[i  ].auxil.∂θ∂t -= sbulk.auxil.q[i] * M_H₂O(FT) / ρ_H₂O(FT) / soils[i  ].auxil.δz;
        soils[i+1].auxil.∂θ∂t += sbulk.auxil.q[i] * M_H₂O(FT) / ρ_H₂O(FT) / soils[i+1].auxil.δz;
    end;
