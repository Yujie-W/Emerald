...
"""
    ...
    leaf_spectra!(config::SPACConfig{FT}, bio::LeafBio{FT}, lwc::FT, θ::FT = FT(40)) where {FT}
...
"""
    ...
    plant_leaf_spectra!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
...
plant_leaf_spectra!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, ρ_par::Number, τ_par::Number, ρ_nir::Number, τ_nir::Number) where {FT} = (
