...
function reflection_spectrum!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    ...
    for i in 1:n_layer
        ...
        sen_i .= sen_geo.auxil.p_sensor[i] .* ilai .* sen_geo.auxil.ko_leaf .* (dob_l .* e_d_i .+ dof_l .* e_u_i) .+
                 sen_geo.auxil.p_sensor[i] .* isai .* sen_geo.auxil.ko_stem .* (dob_s .* e_d_i .+ dof_s .* e_u_i) .+
