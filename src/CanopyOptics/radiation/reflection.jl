...
function reflection_spectrum!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    ...
    for i in 1:n_layer
        ...
        # but the scattered part should use the escape ratio instead of the viewed part, so it should be this:
        sen_i .= sen_geo.auxil.p_sensor[i] ./ sen_geo.auxil.ci_sensor .* ilai .* sen_geo.auxil.ko_leaf .* (dob_l .* e_d_i .+ dof_l .* e_u_i) .+
                 sen_geo.auxil.p_sensor[i] ./ sen_geo.auxil.ci_sensor .* isai .* sen_geo.auxil.ko_stem .* (dob_s .* e_d_i .+ dof_s .* e_u_i) .+
                 sen_geo.auxil.p_sun_sensor[i] .* ilai .* so_l .* rad_sw.e_dir .+
                 sen_geo.auxil.p_sun_sensor[i] .* isai .* so_s .* rad_sw.e_dir;
    end;
