...
"""
    ...
    time_stepper!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::Number) where {FT}
...
"""
function time_stepper!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::Number) where {FT}
    ...
    while δt_remain > 0
        ...
        p_crt = xylem_pressure(bottom_leaf.xylem.trait.vc, config.FEATURES.KR_THRESHOLD) * relative_surface_tension(bottom_leaf.energy.s_aux.t);
