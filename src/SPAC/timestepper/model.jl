...
This function is supposed to have the highest hierarchy, and should support all SPAC types defined in EmeraldNamespace.jl. Note to update the water flow profile when initializing the SPAC.
    ...
    soil_plant_air_continuum!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::Number) where {FT}
...
"""
function soil_plant_air_continuum!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::Number) where {FT}
...
"""
    ...
    spac!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::Number) where {FT}
