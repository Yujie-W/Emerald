...
GOES_R_BRFX(config::SPACConfig{FT}, spac::BulkSPAC{FT}, x::Int) where {FT} = GOES_R_BRFX(spac.canopy, config.CONSTANTS.SPECTRA, x);
...
GOES_R_BRF1(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT} = GOES_R_BRFX(config, spac, 1);

GOES_R_BRF2(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT} = GOES_R_BRFX(config, spac, 2);

GOES_R_BRF3(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT} = GOES_R_BRFX(config, spac, 3);

GOES_R_BRF4(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT} = GOES_R_BRFX(config, spac, 4);
