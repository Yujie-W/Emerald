...
"""
    ...
    regrow_leaves_flag!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
...
"""
function regrow_leaves_flag!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    if !config.FEATURES.ALLOW_LEAF_SHEDDING || !spac.plant._leaf_shedded || spac.plant.pool.c_pool <= 0
        return nothing
    end;

    # do thing if leaf regrowth is not allowed
    if !config.FEATURES.ALLOW_LEAF_REGROWTH
...
#=
function grow_leaves!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, lai_diff::FT) where {FT}
