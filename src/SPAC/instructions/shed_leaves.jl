...
"""
    ...
    shed_leaves!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
...
shed_leaves!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT} = (
    if !config.FEATURES.ALLOW_LEAF_SHEDDING || spac.plant._leaf_shedded
        return nothing
    end;
    ...
    if config.CONFIG_INFO.MESSAGE_LEVEL == 2
