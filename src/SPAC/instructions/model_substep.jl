...
"""
    ...
    substep_preparations!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
...
"""
function substep_preparations!(config::SPACConfig{FT}, spac::BulkSPAC{FT}) where {FT}
    substep_aux!(spac);
    ...
    β_factor!(config, spac);
    plant_photosynthesis!(config, spac);            # the most time consuming function (about 48%)
    stomatal_conductance_profile!(config, spac);    # 2nd most time consuming function (about 32%)
...
"""
    ...
    substep_budgets!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::FT) where {FT}
...
"""
function substep_budgets!(config::SPACConfig{FT}, spac::BulkSPAC{FT}, δt::FT) where {FT}
