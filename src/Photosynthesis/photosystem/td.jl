...
photosystem_temperature_dependence!(
    ...
    psa.j_max  = pst.j_max25 * temperature_correction(config.METHODS.TD_JMAX, t);
    psa.k_c    = temperature_corrected_value(config.METHODS.TD_KC, t);
    psa.k_o    = temperature_corrected_value(config.METHODS.TD_KO, t);
    psa.r_d    = pst.r_d25 * temperature_correction(config.METHODS.TD_R_C3, t);
    psa.v_cmax = pst.v_cmax25 * temperature_correction(config.METHODS.TD_VCMAX_C3, t);
    psa.γ_star = temperature_corrected_value(config.METHODS.TD_Γ, t);
...
photosystem_temperature_dependence!(
            ...
            air::AirLayer{FT},
            t::FT) where {FT} = (
    (; PSI_RATE_CONSTANTS) = config.CONSTANTS;
    (; FIX_ETA_TD) = config.FEATURES;

    psa.k_c    = temperature_corrected_value(config.METHODS.TD_KC, t);
    psa.k_o    = temperature_corrected_value(config.METHODS.TD_KO, t);
    psa.k_q    = temperature_corrected_value(config.METHODS.TD_KQ, t);
    psa.r_d    = pst.r_d25 * temperature_correction(config.METHODS.TD_R_C3, t);
    psa.v_cmax = pst.v_cmax25 * temperature_correction(config.METHODS.TD_VCMAX_C3, t);
    psa.γ_star = temperature_corrected_value(config.METHODS.TD_Γ, t);
    psa.η_c    = temperature_corrected_value(config.METHODS.TD_ηC, t);
    psa.η_l    = temperature_corrected_value(config.METHODS.TD_ηL, t);
    psa.k_m    = psa.k_c * (1 + air.state.p_air * F_O₂(FT) / psa.k_o);
...
photosystem_temperature_dependence!(
    ...
    psa.k_pep_clm = temperature_corrected_value(config.METHODS.TD_KPEP_CLM, t);
    psa.r_d       = pst.r_d25 * temperature_correction(config.METHODS.TD_R_C4, t);
    psa.v_cmax    = pst.v_cmax25 * temperature_correction(config.METHODS.TD_VCMAX_C4, t);
...
photosystem_temperature_dependence!(
    ...
    psa.k_pep  = temperature_corrected_value(config.METHODS.TD_KPEP, t);
    psa.r_d    = pst.r_d25 * temperature_correction(config.METHODS.TD_R_C4, t);
    psa.v_cmax = pst.v_cmax25 * temperature_correction(config.METHODS.TD_VCMAX_C4, t);
    psa.v_pmax = pst.v_pmax25 * temperature_correction(config.METHODS.TD_VPMAX, t);
