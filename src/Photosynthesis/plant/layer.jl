...
leaf_photosynthesis!(
    ...
    photosystem_temperature_dependence!(config, leaf.photosystem, air, leaf.energy.s_aux.t);
    photosystem_electron_transport!(config, cache, leaf.photosystem, leaf.flux.auxil.ppar, leaf.flux.auxil.p_CO₂_i; β = β);
    rubisco_limited_rate!(config, cache, leaf.photosystem, air, leaf.flux.auxil.g_CO₂; β = β);
    light_limited_rate!(config, cache, leaf.photosystem, air, leaf.flux.auxil.g_CO₂; β = β);
    product_limited_rate!(config, cache, leaf.photosystem, air, leaf.flux.auxil.g_CO₂; β = β);
