...
"""
    ...
    clear_legacy!(organ::Union{Leaves2D{FT}, Root{FT}, Stem{FT}}) where {FT}

Clear the legacy for hydraulic organ or system, given
- `spac` `MultiLayerSPAC` type structure
- `organ` `Leaves2D`, `Root`, or `Stem` type structure
"""
...
clear_legacy!(organ::Leaves2D{FT}) where {FT} = clear_legacy!(organ.NS);

clear_legacy!(organ::Union{Root{FT}, Stem{FT}}) where {FT} = clear_legacy!(organ.xylem);
