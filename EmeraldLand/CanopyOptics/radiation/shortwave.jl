...
shortwave_radiation!(config::SPACConfiguration{FT}, spac::BulkSPAC{FT}, ::CanopyLayer{FT}) where {FT} = (
    ...
    for irt in 1:n_layer
        ...
        if can_str.trait.δlai[irt] > 0
            ...
            leaf.flux.auxil.apar[1:end-1] .= sun_geo.s_aux.fs_abs[:] .* Σ_apar_dir .+ Σ_apar_dif;
            ...
            leaf.flux.auxil.ppar[1:end-1] .= sun_geo.s_aux.fs_abs[:] .* Σ_ppar_dir .+ Σ_ppar_dif;
        else
            leaf.flux.auxil.apar .= 0;
            leaf.flux.auxil.ppar .= 0;
        end;
...
shortwave_radiation!(config::SPACConfiguration{FT}, spac::BulkSPAC{FT}, ::Leaf{FT}) where {FT} = (
    can_str = spac.canopy.structure;
    leaves = spac.plant.leaves;
    sbulk = spac.soil_bulk;
    sun_geo = spac.canopy.sun_geometry;
