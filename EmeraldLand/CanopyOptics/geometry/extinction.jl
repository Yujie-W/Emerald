...
extinction_coefficient(θ_incl::FT, ci::ClumpingIndex{FT}) where {FT} = (
    # compute the mean extinction coefficient for diffuse solar radiation from 18 angles
    Σk::FT = 0;
    Σs::FT = 0;
    for θ_sza in 0:1:89.9
        Σk += extinction_coefficient(θ_sza + FT(0.5), θ_incl) * sind(θ_sza + FT(0.5)) * ci.ci_0 * (1 - ci.ci_1 * cosd(θ_sza + FT(0.5)));
        Σs += sind(θ_sza + FT(0.5));
    end;
