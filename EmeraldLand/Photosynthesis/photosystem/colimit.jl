...
colimited_rate(a_1::Union{FT, Vector{FT}}, a_2::Vector{FT}, a_i::Vector{FT}, colim::MinimumColimit{FT}) where {FT} = (a_i .= min.(a_1, a_2); return nothing);

colimited_rate(a_1::FT, a_2::FT, colim::QuadraticColimit{FT}) where {FT} = lower_quadratic(colim.CURVATURE, -(a_1 + a_2), a_1 * a_2);

colimited_rate(a_1::Union{FT, Vector{FT}}, a_2::Vector{FT}, a_i::Vector{FT}, colim::QuadraticColimit{FT}) where {FT} = (
...
colimited_rate(a_1::Union{FT, Vector{FT}}, a_2::Vector{FT}, a_i::Vector{FT}, colim::SerialColimit{FT}) where {FT} = (a_i .= a_1 .* a_2 ./ (a_1 .+ a_2); return nothing);

colimited_rate(a_1::FT, a_2::FT, colim::SquareColimit{FT}) where {FT} = a_1 * a_2 / sqrt(a_1 ^ 2 + a_2 ^ 2);

colimited_rate(a_1::Union{FT, Vector{FT}}, a_2::Vector{FT}, a_i::Vector{FT}, colim::SquareColimit{FT}) where {FT} = (a_i .= a_1 .* a_2 / sqrt.(a_1 .^ 2 .+ a_2 .^ 2); return nothing);
