...
leaf_photosynthesis!(leaf::CanopyLayer{FT}, air::AirLayer{FT}, mode::GCO₂Mode, β::FT; rd_only::Bool = false) where {FT} = (
    if rd_only
    ...
    photosystem_electron_transport!(leaf.photosystem, [leaf.flux.auxil.ppar_sunlit[:]; leaf.flux.auxil.ppar_shaded], [leaf.flux.auxil.p_CO₂_i_sunlit[:]; leaf.flux.auxil.p_CO₂_i_shaded]; β = β);
    rubisco_limited_rate!(leaf.photosystem, air, [leaf.flux.auxil.g_CO₂_sunlit[:]; leaf.flux.auxil.g_CO₂_shaded]; β = β);
    light_limited_rate!(leaf.photosystem, air, [leaf.flux.auxil.g_CO₂_sunlit[:]; leaf.flux.auxil.g_CO₂_shaded]; β = β);
    product_limited_rate!(leaf.photosystem, air, [leaf.flux.auxil.g_CO₂_sunlit[:]; leaf.flux.auxil.g_CO₂_shaded]; β = β);
    ...
    p_CO₂_i = air.s_aux.ps[2] .- leaf.photosystem.auxil.a_n ./ [leaf.flux.auxil.g_CO₂_sunlit[:]; leaf.flux.auxil.g_CO₂_shaded] .* air.state.p_air .* FT(1e-6);
    p_CO₂_s = air.s_aux.ps[2] .- leaf.photosystem.auxil.a_n ./ leaf.flux.auxil.g_CO₂_b .* air.state.p_air .* FT(1e-6);
    leaf.flux.auxil.p_CO₂_i_sunlit[:] .= p_CO₂_i[1:end-1];
    leaf.flux.auxil.p_CO₂_s_sunlit[:] .= p_CO₂_s[1:end-1];
    leaf.flux.auxil.p_CO₂_i_shaded = p_CO₂_i[end];
    leaf.flux.auxil.p_CO₂_s_shaded = p_CO₂_s[end];

    # update leaf ETR again to ensure that j_pot and e_to_c are correct for C3CytochromeModel
    photosystem_electron_transport!(leaf.photosystem, [leaf.flux.auxil.ppar_sunlit[:]; leaf.flux.auxil.ppar_shaded], p_CO₂_i; β = β);

    # update the fluorescence related parameters
    photosystem_coefficients!(leaf.photosystem, [leaf.flux.auxil.ppar_sunlit[:]; leaf.flux.auxil.ppar_shaded]; β = β);

    # save the rates and to leaf
    leaf.flux.auxil.a_n_sunlit[:] .= leaf.photosystem.auxil.a_n[1:end-1];
    leaf.flux.auxil.a_g_sunlit[:] .= leaf.photosystem.auxil.a_g[1:end-1];
    leaf.flux.auxil.etr_sunlit[:] .= leaf.photosystem.auxil.a_g[1:end-1] ./ leaf.photosystem.auxil.e2c[1:end-1];
    leaf.flux.auxil.ϕ_d_sunlit[:] .= leaf.photosystem.auxil.ϕ_d[1:end-1];
    leaf.flux.auxil.ϕ_f_sunlit[:] .= leaf.photosystem.auxil.ϕ_f[1:end-1];
    leaf.flux.auxil.ϕ_n_sunlit[:] .= leaf.photosystem.auxil.ϕ_n[1:end-1];
    leaf.flux.auxil.ϕ_p_sunlit[:] .= leaf.photosystem.auxil.ϕ_p[1:end-1];
    leaf.flux.auxil.a_n_shaded = leaf.photosystem.auxil.a_n[end];
    leaf.flux.auxil.a_g_shaded = leaf.photosystem.auxil.a_g[end];
    leaf.flux.auxil.etr_shaded = leaf.photosystem.auxil.a_g[end] / leaf.photosystem.auxil.e2c[end];
    leaf.flux.auxil.ϕ_d_shaded = leaf.photosystem.auxil.ϕ_d[end];
    leaf.flux.auxil.ϕ_f_shaded = leaf.photosystem.auxil.ϕ_f[end];
    leaf.flux.auxil.ϕ_n_shaded = leaf.photosystem.auxil.ϕ_n[end];
    leaf.flux.auxil.ϕ_p_shaded = leaf.photosystem.auxil.ϕ_p[end];

    return nothing
);

# This method computes and save the photosynthetic rates into leaf flux struct for PCO₂Mode
leaf_photosynthesis!(leaf::CanopyLayer{FT}, air::AirLayer{FT}, mode::PCO₂Mode, β::FT; rd_only::Bool = false) where {FT} = (
    if rd_only
        leaf.photosystem.auxil.r_d   = leaf.photosystem.trait.r_d25 * temperature_correction(leaf.photosystem.trait.TD_R, leaf.energy.s_aux.t);
        leaf.flux.auxil.a_n_sunlit  .= -leaf.photosystem.auxil.r_d;
        leaf.flux.auxil.a_g_sunlit  .= 0;
