...
adjusted_time(plant::Plant{FT}, lai::FT, δt::FT) where {FT} = (
    ...
    for leaf in plant.leaves
        for ∂g∂t in leaf.flux.auxil.∂g∂t_sunlit
            ...
            if isnan(new_δt) || new_δt < 0.001 <= δt
                @error "NaN or very small δt detected when adjusting δt based on sunlit leaf stomatal conductance dYdt" ∂g∂t;
                return error("NaN detected in adjusted_time")
            end;
        end;
        new_δt = min(FT(0.01) / abs(leaf.flux.auxil.∂g∂t_shaded), new_δt);
        if isnan(new_δt) || new_δt < 0.001 <= δt
            @error "NaN or very small δt detected when adjusting δt based on shaded leaf stomatal conductance dYdt" leaf.flux.auxil.∂g∂t_shaded;
            return error("NaN detected in adjusted_time")
        end;
    end;
