...
empirical_equation(sm::MedlynSM{FT}, leaf::Leaf{FT}, air::AirLayer{FT}; β::FT = FT(1)) where {FT} = (
    ...
    return G0 + FT(1.6) * (1 + β * G1 / sqrt(d)) * leaf.flux.auxil.a_n_shaded * FT(1e-6) / air.auxil.ps[2] * air.state.p_air
);
...
empirical_equation(sm::MedlynSM{FT}, leaf::Leaf{FT}, air::AirLayer{FT}, ind::Int; β::FT = FT(1)) where {FT} = (
    ...
    return G0 + FT(1.6) * (1 + β * G1 / sqrt(d)) * leaf.flux.auxil.a_n_sunlit[ind] * FT(1e-6) / air.auxil.ps[2] * air.state.p_air
);
