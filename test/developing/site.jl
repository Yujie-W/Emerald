...
for i in 1:100
    EmeraldLand.SPAC.spac!(config, spac, FT(360));
    tswc = sum([soil.θ * soil.t_aux.δz for soil in spac.SOIL.LAYERS]);
    soil_water_flow = sum([soil.∂θ∂t * soil.t_aux.δz for soil in spac.SOIL.LAYERS]);
    ...
    pns = [soil.state.ns[4] / (soil.t_aux.δz * max(0, soil.VC.Θ_SAT - soil.θ)) / 1000 * soil.t * EmeraldLand.Constant.GAS_R() for soil in spac.SOIL.LAYERS];
